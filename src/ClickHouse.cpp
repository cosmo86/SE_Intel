#include "../include//ClickHouse.hpp"
    ...
    bool ClickHouse::Init_CH(){
        buildFunctions["OnRspSubNGTSTick"]=[this](){ buildNGTSTick();};
        buildFunctions["OnRspSubTransaction"]=[this](){ buildTransaction();};
        buildFunctions["OnRspSubOrderDetail"]=[this](){ buildOrderDetail();};
        return true;
    }
    bool ClickHouse::build(std::string sign) {
        // 在 build 函数中根据 sign 调用对应的处理函数
        auto it = buildFunctions.find(sign);
        if (it != buildFunctions.end()) {
            it->second(); // 调用对应的处理函数
        } else {
            return false; // 处理函数不存在，返回 false 或者进行其他错误处理
        }
        return true;
    }
